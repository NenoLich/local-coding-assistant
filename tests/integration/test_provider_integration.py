...
class TestProviderCLIIntegration:
    ...
    def test_add_provider_via_cli(self, temp_config_dir, sample_provider_config):
        ...
        with patch("pathlib.Path.home") as mock_home:
            ...
            with patch("local_coding_assistant.cli.commands.provider.bootstrap") as mock_bootstrap:
                ...
                assert "test_provider" in saved_config
                assert saved_config["test_provider"]["driver"] == "openai_chat"
                assert saved_config["test_provider"]["api_key"] == "test-key"
                assert saved_config["test_provider"]["base_url"] == "https://api.example.com"
                assert "gpt-4" in saved_config["test_provider"]["models"]
                assert "gpt-3.5-turbo" in saved_config["test_provider"]["models"]
    ...
    def test_remove_provider_via_cli(self, temp_config_dir):
        ...
        with patch("pathlib.Path.home") as mock_home:
            ...
            with patch("local_coding_assistant.cli.commands.provider.bootstrap") as mock_bootstrap:
                ...
                assert "google" in updated_config
    ...
    def test_validate_provider_config(self, temp_config_dir):
        ...
        invalid_config = {
            "bad_provider": {"invalid_field": "value"},  # Missing required 'driver' field
        }
        ...
        with patch("pathlib.Path.home") as mock_home:
            ...
            # Should complete validation (warnings are expected)
            validate(config_file=str(config_file))
...
class TestProviderYAMLConfiguration:
    ...
    def test_load_providers_from_global_yaml(self, global_config_dir, mock_provider_manager):
    ...
    def test_load_providers_from_local_yaml(self, local_config_dir, mock_provider_manager):
        ...
        with patch("pathlib.Path.home") as mock_home:
            ...
            assert mock_provider_manager.get_provider_source("custom_provider") == "global"

    def test_provider_config_layer_priority(self, global_config_dir, local_config_dir, mock_provider_manager):
        ...
        with patch("local_coding_assistant.providers.provider_manager.Path") as mock_path:
            ...
            with patch("pathlib.Path.home") as mock_home:
                ...
                provider_config = mock_provider_manager._provider_configs.get("openai", {})
    ...
    def test_builtin_provider_auto_discovery(self, mock_provider_manager):
        ...
        for provider in builtin_providers:
            if provider in providers:
                ...
                assert source == "builtin" or source == "global", f"Provider {provider} should be builtin or global"
...
'''
            ...
            try:
                ...
                spec = importlib.util.spec_from_file_location("custom_provider", provider_file)
                if spec and spec.loader:
                    ...
                    if "custom" in providers:
                        ...
                        assert source == "builtin"  # Should be marked as builtin when auto-discovered
    ...
    def test_provider_manager_with_llm_manager_integration(self, mock_provider_manager):
        ...
        with patch("local_coding_assistant.config.get_config_manager") as mock_get_config:
            with patch("local_coding_assistant.agent.llm_manager.ProviderManager") as mock_pm_class:
                ...
                llm_manager = LLMManager(config_manager=mock_config_manager, provider_manager=mock_provider_manager)
