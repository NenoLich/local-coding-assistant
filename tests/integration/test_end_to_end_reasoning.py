...
class TestEndToEndReasoningChain:
    ...
    async def test_end_to_end_reasoning_with_tool_calls(
        ...
        assert "New York" in result["final_answer"] or "pack" in result["final_answer"].lower()
    ...
    async def test_session_persistence_across_queries(self, runtime_manager):
        ...
        assert result1["session_id"] == result2["session_id"], "Session ID should be the same"
    ...
    def _assert_valid_agent_response(self, response: Dict[str, Any]) -> None:
        ...
        assert all(key in response for key in required_keys), "Missing required response keys"
    ...
    def _extract_tool_calls(self, result: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for iteration in result["history"]:
            if (iteration.get("action_result") and 
                iteration["action_result"].get("metadata", {}).get("tool_calls")):
