...
from local_coding_assistant.runtime.handlers.handler_types import HandlerContext, HandlerOutput
...
class TestRuntimeManagerRegularModeHandlerIntegration:
    ...
    def mock_config_manager(self):
        ...
        config_manager.load_global_config = MagicMock(return_value=config_manager.global_config)
    ...
    @pytest.fixture
    def runtime_manager(self, mock_config_manager, mock_llm_service_partial_response, mock_tool_manager):
    ...
    @pytest.mark.asyncio
    async def test_handler_integration_continue_execution(self, mock_handler_output_continue):
        """Test that handler integration continues execution when should_continue returns True."""
        from local_coding_assistant.config.schemas import AppConfig, LLMConfig, RuntimeConfig

        mock_handler_integration = MagicMock()
        mock_handler_integration.handle_partial_response = AsyncMock(return_value=mock_handler_output_continue)
        mock_handler_integration.should_continue_execution = MagicMock(return_value=True)
        mock_handler_integration.get_adjusted_llm_options = MagicMock(return_value={"max_tokens": 1500})
        ...
        report = await runtime_manager.orchestrate("Test query that triggers partial response")
        ...
        assert handler_context.raw_response == "This is a partial response that was truncated due to token limits."
        assert handler_context.reasoning == "This is reasoning that explains the partial response."
        ...
        mock_handler_integration.should_continue_execution.assert_called_once_with(mock_handler_output_continue)
    ...
    async def test_handler_integration_fail_execution(self, mock_handler_output_fail):
        ...
        mock_handler_integration = MagicMock()
        mock_handler_integration.handle_partial_response = AsyncMock(return_value=mock_handler_output_fail)
        mock_handler_integration.should_continue_execution = MagicMock(return_value=False)
        ...
        mock_handler_integration.should_continue_execution.assert_called_once_with(mock_handler_output_fail)
    ...
    async def test_handler_context_creation(self, mock_handler_output_continue):
        ...
        assert handler_context.error_type.value == "truncation"
        assert handler_context.error_message == "Unknown error"  # Default when not in metadata
        assert handler_context.raw_response == "This is a partial response that was truncated due to token limits."
        assert handler_context.raw_tool_calls == []  # No tool calls in the mock

    @pytest.mark.asyncio
    async def test_llm_options_adjustment_integration(self, mock_handler_output_continue):
        ...
        report = await runtime_manager.orchestrate("Test query")
