...
class TestDashboardServeCommand:
    ...
    def test_serve_detached_mode(self, mock_start_detached):
        ...
        assert result.exit_code == 0
        mock_start_detached.assert_called_once_with(
            "127.0.0.1",
            8080,
            False,
            "INFO"
...
class TestServerCommandBuilder:
    ...
    def test_build_server_command_default(self):
        ...
        expected = [
            sys.executable,
            str(Path(__file__).parent.parent.parent.parent / "src" / "local_coding_assistant" / "dashboard" / "run_dashboard.py"),
            "--host",
            "127.0.0.1",
            "--port",
            "8080",
...
class TestSubprocessManagement:
    ...
    @patch('time.sleep')
    @patch('typer.echo')
    def test_start_detached_server_success(self, mock_echo, mock_sleep, mock_launch, mock_build):
    ...
    @patch('time.sleep')
    @patch('typer.echo')
    def test_start_detached_server_failure(self, mock_echo, mock_sleep, mock_launch, mock_build):
    ...
    @patch.dict('sys.modules', {
        'uvicorn': Mock(),
        'local_coding_assistant.dashboard.app': Mock()
    })
    @patch('typer.echo')
...
class TestDashboardProcessManager:
    ...
    def test_find_all_dashboard_processes(self, mock_iter):
        ...
        with patch.object(DashboardProcessManager, 'is_dashboard_process', side_effect=lambda x: 'run_dashboard.py' in x):
    ...
    def test_find_dashboard_processes_on_port(self, mock_iter):
        ...
        with patch.object(DashboardProcessManager, 'is_dashboard_process', return_value=True):
    ...
    def test_find_any_processes_on_port(self, mock_iter):
        ...
        mock_proc.info = {
            'pid': 456,
            'connections': [Mock(laddr=Mock(port=8080))]
        }
        mock_proc.pid = 456
...
class TestStopFunctions:
    ...
    def test_stop_all_dashboard_servers(self, mock_echo):
        ...
        with patch('local_coding_assistant.cli.commands.dashboard._stop_all_dashboards') as mock_stop:
    ...
    def test_stop_dashboard_on_port(self, mock_echo):
        ...
        with patch.object(DashboardProcessManager, 'find_dashboard_processes_on_port') as mock_find:
            with patch.object(ProcessTerminator, 'terminate_process', return_value=True) as mock_terminate:
                mock_proc = Mock()
                mock_proc.pid = 789
    ...
    def test_stop_dashboard_on_port_fallback(self, mock_echo):
        ...
        with patch.object(DashboardProcessManager, 'find_dashboard_processes_on_port', return_value=[]):
            with patch.object(DashboardProcessManager, 'find_any_processes_on_port') as mock_find_any:
                with patch.object(ProcessTerminator, 'terminate_process', return_value=True) as mock_terminate:
                    mock_proc = Mock()
                    mock_proc.pid = 999
