...
from local_coding_assistant.dashboard.routes.websocket import ConnectionManager
...
class TestAPIRoutes:
    ...
    def test_get_frame_detail(self):
        ...
        assert (
            data["run_id"] == frame_id
        )  # In current implementation, frame_id == run_id
...
class TestWebSocketRoutes:
    ...
    async def test_connection_manager_connect(self):
        ...
        with patch(
            ...
            assert (
                self.mock_websocket.send_text.call_count == 3
            )  # stats, activity, sessions
