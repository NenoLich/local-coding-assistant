...
class TestConfigManager:
    ...
    def test_init(self):
        ...
        with pytest.raises(ConfigError):
            _ = manager.global_config

        with pytest.raises(ConfigError):
            _ = manager.session_overrides
    ...
    def test_set_session_overrides(self):
        ...
        assert manager.session_overrides == overrides
    ...
    def test_clear_session_overrides(self):
        ...
        assert manager.session_overrides != {}
    ...
    def test_reload_tools_failure(self):
        ...
        with patch.object(manager, "_load_tools", side_effect=Exception("Load failed")):
            with patch(
                "local_coding_assistant.config.config_manager.logger"
            ) as mock_logger:
                with pytest.raises(
                    ConfigError, match="Failed to reload tools: Load failed"
                ):
                    manager.reload_tools()
    ...
    def test_set_session_overrides_without_global_config(self):
        ...
        # Valid overrides but no global config - this should raise ConfigError
        overrides = {"llm.temperature": 0.5}
    ...
    def test_save_config_no_global_config_loaded(self):
        ...
        with pytest.raises(
            ConfigError,
            match="No configuration loaded. Call load_global_config\\(\\) first.",
        ):
            manager.save_config()
    ...
    def test_save_config_success_with_path(self):
        ...
        try:
            ...
            with patch.object(
                manager._path_manager, "resolve_path", return_value=Path(temp_path)
            ):
    ...
    def test_save_config_file_write_error(self):
        ...
        with patch.object(
            manager._path_manager, "resolve_path", return_value=Path("/tmp/test.yaml")
        ):
            with patch("builtins.open", side_effect=OSError("Permission denied")):
                with pytest.raises(
                    ConfigError,
                    match="Failed to save configuration to /tmp/test.yaml: Permission denied",
                ):
    ...
    def test_save_config_yaml_error(self):
        ...
        with patch.object(
            ...
            with patch("builtins.open", mock_open()):
                with patch(
                    "yaml.safe_dump", side_effect=yaml.YAMLError("Serialization failed")
                ):
                    with pytest.raises(
                        ConfigError,
                        match="Failed to save configuration to /tmp/test.yaml: Serialization failed",
                    ):
                        manager.save_config("/tmp/test.yaml")
