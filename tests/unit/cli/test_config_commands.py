...
from local_coding_assistant.cli.commands.config import app, get_config, set_config, unset_config, _env_manager, _k
...
class TestConfigCommands:
    ...
    def setup_teardown(self):
        ...
        self.env_paths = [Path(self.temp_dir.name) / ".env", Path(self.temp_dir.name) / ".env.local"]
        ...
        self.test_env_manager.get_env.side_effect = lambda k: os.environ.get(self.test_env_manager.with_prefix(k))
    ...
    def test_get_all_config(self):
        ...
        with patch('local_coding_assistant.cli.commands.config.os.environ', 
                  {f"{TEST_PREFIX}KEY1": "value1"}):
