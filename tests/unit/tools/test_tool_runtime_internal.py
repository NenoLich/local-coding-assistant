...
from local_coding_assistant.tools.tool_manager import ToolInputTransformer, ToolRuntime
...
def test_ensure_mapping_variants(runtime: ToolRuntime) -> None:
    assert runtime._ensure_mapping({"value": 1}) == {"value": 1}
    assert runtime._ensure_mapping(SampleInput(value=2)) == {"value": 2}
    assert runtime._ensure_mapping(json.dumps({"value": 3})) == {"value": 3}
    assert runtime._ensure_mapping([("value", 4)]) == {"value": 4}

    with pytest.raises(ValueError):
        runtime._ensure_mapping(json.dumps([1, 2]))

    with pytest.raises(ValueError):
        runtime._ensure_mapping(object())


def test_transform_payload_delegates(runtime: ToolRuntime, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_validate_payload_against_model_with_instance(runtime: ToolRuntime) -> None:
    instance = SampleInput(value=4)
    assert runtime._validate_payload_against_model(instance, SampleInput) is instance


def test_validate_payload_against_model_converts_dict(runtime: ToolRuntime) -> None:
    validated = runtime._validate_payload_against_model({"value": 6}, SampleInput)
    assert isinstance(validated, SampleInput)
    assert validated.value == 6


def test_validate_payload_against_model_falls_back_to_schema(
    runtime: ToolRuntime, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setattr(
        runtime,
    ...
    payload = {"value": "5"}
    result = runtime._validate_payload_against_model(payload, SampleInput)
    assert result == SampleInput(value=5)


def test_validate_payload_against_model_raises_when_schema_fails(
    runtime: ToolRuntime, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    with pytest.raises(ValidationError):
        runtime._validate_payload_against_model({"value": object()}, SampleInput)
...
def test_validate_field_type_variants(runtime: ToolRuntime) -> None:
    assert runtime._validate_field_type("x", "string|number", {}) is True
    assert runtime._validate_field_type(5, "string|number", {}) is True

    schema = {"type": "array", "items": {"type": "integer"}}
    assert runtime._validate_field_type([1, 2], schema["type"], schema) is True
    assert runtime._validate_field_type(["a"], schema["type"], schema) is False


def test_validate_with_parameters_schema_success(runtime: ToolRuntime) -> None:
    payload = runtime._validate_with_parameters_schema(
        {"value": 3},
...
def test_validate_with_parameters_schema_errors(runtime: ToolRuntime) -> None:
    ...
    with pytest.raises(ValueError):
        runtime._validate_with_parameters_schema(
            {"value": "oops"},
            runtime.info.parameters,
        )
    ...
    with pytest.raises(ValueError):
        runtime._validate_with_parameters_schema({"mode": "other"}, schema)


def test_prepare_input_success(runtime: ToolRuntime, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(runtime.instance, "Input", SampleInput)
    payload = runtime._prepare_input({"value": 10})
    assert isinstance(payload, SampleInput)
    assert payload.value == 10


def test_prepare_input_model_error(runtime: ToolRuntime, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def mock_validate(cls, data: dict) -> SampleInput:
    ...
    try:
        with pytest.raises(ToolRegistryError) as exc:
            runtime._prepare_input({"value": "bad"})
...
def test_prepare_input_schema_error(runtime: ToolRuntime) -> None:
    runtime.has_input_validation = False
    runtime.info.parameters = {
        "type": "object",
        "properties": {"value": {"type": "integer"}},
...
@pytest.mark.asyncio
async def test_invoke_run_sync_tool(runtime: ToolRuntime, monkeypatch: pytest.MonkeyPatch) -> None:
...
@pytest.mark.asyncio
async def test_invoke_run_async_tool(runtime: ToolRuntime, monkeypatch: pytest.MonkeyPatch) -> None:
...
@pytest.mark.asyncio
async def test_invoke_run_missing_runner(runtime: ToolRuntime, monkeypatch: pytest.MonkeyPatch) -> None:
...
@pytest.mark.asyncio
async def test_invoke_stream_requires_tool_kind(runtime: ToolRuntime, monkeypatch: pytest.MonkeyPatch) -> None:
...
@pytest.mark.asyncio
async def test_invoke_stream_requires_support(runtime: ToolRuntime, monkeypatch: pytest.MonkeyPatch) -> None:
    # Save the original method
    original_invoke_stream = runtime._invoke_stream
...
def test_validate_with_model_variants(runtime: ToolRuntime) -> None:
    assert runtime._validate_with_model(SampleOutput, None) == {"result": 0}
    assert runtime._validate_with_model(SampleOutput, {"result": 3}) == {"result": 3}
    ...
    assert runtime._validate_with_model(SampleOutput, Dumping(4)) == {"result": 4}
    ...
    assert runtime._validate_with_model(SampleOutput, DictLike(5)) == {"result": 5}

    class IterableResult:
        def __iter__(self):
            yield ("result", 6)

    assert runtime._validate_with_model(SampleOutput, IterableResult()) == {"result": 6}
    assert runtime._validate_with_model(SampleOutput, 7) == {"result": 7}


def test_normalize_output_without_validation(runtime: ToolRuntime) -> None:
    runtime.has_output_validation = False
    assert runtime._normalize_output(SampleOutput(result=8)) == {"result": 8}


def test_normalize_output_without_output_model(runtime: ToolRuntime, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(runtime.instance, "Output", None, raising=False)
    assert runtime._normalize_output({"raw": 1}) == {"raw": 1}


def test_normalize_output_with_instance(runtime: ToolRuntime) -> None:
    assert runtime._normalize_output(SampleOutput(result=9)) == {"result": 9}


def test_normalize_output_falls_back_on_error(runtime: ToolRuntime, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        runtime,
    ...
    marker = object()
    assert runtime._normalize_output(marker) is marker
