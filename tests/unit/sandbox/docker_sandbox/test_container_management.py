...
async def test_cleanup_container_success(sandbox_project_root: Path) -> None:
    ...
    with (
        patch.object(sandbox, "stop_container", new_callable=AsyncMock) as mock_stop,
        patch("shutil.rmtree") as mock_rmtree,
    ):
        await sandbox._cleanup_container("test-session", mock_container, remove=True)

        mock_stop.assert_called_once_with(
            "container-id", remove=True, session_id="test-session"
        )
...
async def test_get_existing_container_success(sandbox_project_root: Path) -> None:
    ...
    with patch.object(sandbox, "_should_reuse_running_container", return_value=True):
        result = await sandbox._reuse_existing_named_container(
            "session1", "test-container", True
        )
...
@pytest.mark.asyncio
async def test_get_existing_container_reload_failure(
    sandbox_project_root: Path,
) -> None:
...
@pytest.mark.asyncio
async def test_get_existing_container_should_not_reuse(
    sandbox_project_root: Path,
) -> None:
    ...
    with (
        patch.object(sandbox, "_should_reuse_running_container", return_value=False),
        patch.object(
            sandbox, "_cleanup_container", new_callable=AsyncMock
        ) as mock_cleanup,
    ):
        result = await sandbox._reuse_existing_named_container(
            "session1", "test-container", True
...
@pytest.mark.asyncio
async def test_start_session_success_with_new_container(
    sandbox_project_root: Path,
) -> None:
    ...
    with (
        patch.object(
            sandbox, "_ensure_initialized", new_callable=AsyncMock
        ) as mock_init,
        patch.object(
            sandbox,
            "_reuse_tracked_container",
            new_callable=AsyncMock,
            return_value=None,
        ) as mock_reuse,
        patch.object(
            sandbox, "_ensure_image_ready", new_callable=AsyncMock
        ) as mock_image,
        ...
        patch.object(
            sandbox, "_determine_container_parameters", return_value=("test-name", None)
        ) as mock_params,
        ...
        patch.object(
            sandbox,
            "_reuse_existing_named_container",
            new_callable=AsyncMock,
            return_value=None,
        ) as mock_named_reuse,
        patch.object(
            sandbox, "_create_container", return_value=mock_container
        ) as mock_create,
    ):
...
@pytest.mark.asyncio
async def test_start_session_success_with_reused_container(
    sandbox_project_root: Path,
) -> None:
...
@pytest.mark.asyncio
async def test_start_session_persistent_validates_capacity(
    sandbox_project_root: Path,
) -> None:
    ...
    with (
        patch.object(sandbox, "_ensure_initialized", new_callable=AsyncMock),
        patch.object(
            sandbox,
            "_reuse_tracked_container",
            new_callable=AsyncMock,
...
@pytest.mark.asyncio
async def test_start_session_with_named_container_reuse(
    sandbox_project_root: Path,
) -> None:
...
@pytest.mark.asyncio
async def test_start_session_container_creation_failure(
    sandbox_project_root: Path,
) -> None:
    ...
    with (
        ...
        patch.object(
            sandbox, "_create_container", side_effect=Exception("Creation failed")
        ) as mock_create,
    ):
...
@pytest.mark.asyncio
async def test_pop_tracked_container_with_session_id(
    sandbox_project_root: Path,
) -> None:
...
@pytest.mark.asyncio
async def test_pop_tracked_container_with_container_id(
    sandbox_project_root: Path,
) -> None:
    ...
    with patch.object(
        sandbox, "_find_session_id_by_container", return_value="test-session"
    ) as mock_find:
...
async def test_pop_tracked_container_not_found(sandbox_project_root: Path) -> None:
    ...
    with patch.object(
        sandbox, "_find_session_id_by_container", return_value=None
    ) as mock_find:
        session_id, container = sandbox._pop_tracked_container(
...
async def test_stop_container_once_not_found(sandbox_project_root: Path) -> None:
    ...
    result = sandbox._stop_container_once(
        mock_container, "container-123", "test-session", False
    )
...
async def test_stop_container_once_already_exited(sandbox_project_root: Path) -> None:
    ...
    result = sandbox._stop_container_once(
        mock_container, "container-123", "test-session", True
    )
...
@pytest.mark.asyncio
async def test_stop_container_once_running_stop_only(
    sandbox_project_root: Path,
) -> None:
...
@pytest.mark.asyncio
async def test_stop_container_once_running_stop_and_remove(
    sandbox_project_root: Path,
) -> None:
...
async def test_stop_container_success(sandbox_project_root: Path) -> None:
    ...
    with (
        ...
        patch.object(
            sandbox,
            "_pop_tracked_container",
            return_value=("test-session", mock_container),
        ) as mock_pop,
        patch.object(
            sandbox, "_stop_container_once", return_value=True
        ) as mock_stop_once,
    ):
        await sandbox.stop_container(
            "container-123", remove=False, session_id="test-session", max_retries=3
        )

        mock_init.assert_called_once()
        mock_pop.assert_called_once_with("test-session", "container-123")
        mock_stop_once.assert_called_once_with(
            mock_container, "container-123", "test-session", False
        )
...
async def test_stop_container_not_found(sandbox_project_root: Path) -> None:
    ...
    with (
        ...
        patch.object(
            sandbox, "_pop_tracked_container", return_value=(None, None)
        ) as mock_pop,
    ):
...
async def test_stop_container_with_retries(sandbox_project_root: Path) -> None:
    ...
    api_error.__str__ = (
        lambda: "409 Client Error: Conflict (container removal already in progress)"
    )
    ...
    with (
        ...
        patch.object(
            sandbox, "_stop_container_once", side_effect=[api_error, True]
        ) as mock_stop_once,
        patch("asyncio.sleep", new_callable=AsyncMock) as mock_sleep,
    ):
        await sandbox.stop_container(
            "container-123", remove=False, session_id="test-session", max_retries=2
        )
...
async def test_stop_container_max_retries_exceeded(sandbox_project_root: Path) -> None:
    ...
    with (
        ...
        patch.object(
            sandbox, "_stop_container_once", side_effect=Exception("Always fails")
        ) as mock_stop_once,
        patch("asyncio.sleep", new_callable=AsyncMock) as mock_sleep,
    ):
