...
@pytest.mark.asyncio
async def test_check_docker_available_connection_failure(
...
@pytest.mark.asyncio
async def test_check_docker_available_client_close_error(
    sandbox_project_root: Path,
) -> None:
...
async def test_discover_existing_containers_success(sandbox_project_root: Path) -> None:
    ...
    sandbox = DockerSandbox(
        path_manager=FakePathManager(sandbox_project_root), image="test-image"
    )
...
@pytest.mark.asyncio
async def test_discover_existing_containers_no_client(
    sandbox_project_root: Path,
) -> None:
...
@pytest.mark.asyncio
async def test_discover_existing_containers_exception(
    sandbox_project_root: Path,
) -> None:
...
async def test_initialize_sandbox_success(sandbox_project_root: Path) -> None:
    ...
    with (
        patch.object(
            sandbox, "_ensure_client", new_callable=AsyncMock
        ) as mock_ensure_client,
        patch.object(
            sandbox, "_discover_existing_containers", new_callable=AsyncMock
        ) as mock_discover,
        patch.object(
            sandbox, "ensure_directories", new_callable=AsyncMock
        ) as mock_dirs,
    ):
...
@pytest.mark.asyncio
async def test_initialize_sandbox_already_initialized(
    sandbox_project_root: Path,
) -> None:
...
async def test_ensure_client_failure(sandbox_project_root: Path) -> None:
    ...
    with patch("docker.from_env", return_value=mock_client):
        with pytest.raises(
            SandboxRuntimeError, match="Docker is not running or not installed"
        ):
            await sandbox._ensure_client()
