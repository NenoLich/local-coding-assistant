...
@pytest.mark.asyncio
async def test_ensure_image_available_not_found_no_auto_build(
    ...
    with pytest.raises(
        SandboxRuntimeError, match="Image test-image not found and auto_build is False"
    ):
        await sandbox._ensure_image_ready()


@pytest.mark.asyncio
async def test_ensure_image_available_not_found_with_auto_build(
    sandbox_project_root: Path,
) -> None:
...
def test_has_docker_changes_no_changes(sandbox_project_root: Path) -> None:
    ...
    with patch.object(
        sandbox, "_get_docker_context_files", return_value=[mock_file1, mock_file2]
    ) as mock_files:
...
def test_has_docker_changes_with_file_modification(sandbox_project_root: Path) -> None:
    ...
    with patch.object(
        sandbox, "_get_docker_context_files", return_value=[mock_file]
    ) as mock_files:
        result = sandbox._has_docker_changes()
...
def test_has_docker_changes_python_version_fallback(sandbox_project_root: Path) -> None:
    ...
    with patch.object(
        sandbox, "_get_docker_context_files", return_value=[]
    ) as mock_files:
        result = sandbox._has_docker_changes()
...
def test_has_docker_changes_exception_assumes_changes(
    sandbox_project_root: Path,
) -> None:
