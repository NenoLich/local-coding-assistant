...
class TestCLIErrorHandling:
    ...
    def test_cli_invalid_option(self, cli_runner):
        ...
        result = cli_runner.invoke(
            app, ["run", "query", "test", "--invalid-option"], catch_exceptions=False
        )
        ...
        # Check that help is suggested (the actual format might be different)
        assert (
            "--help" in error_output or "Try 'root run query --help'" in error_output
        ), f"Help suggestion not found in: {error_output}"
...
class TestCLIIntegration:
    ...
    def test_cli_environment_variable_precedence(self, cli_runner, mock_env_vars):
        ...
        result = cli_runner.invoke(
            app, ["run", "query", "test"], catch_exceptions=False
        )
        ...
        assert result.exit_code == 0, (
            f"Command failed with DEBUG level: {result.stdout} {result.stderr}"
        )
    ...
    def test_cli_config_persistence_across_commands(self, cli_runner):
        ...
        try:
            # Set a config value
            result = cli_runner.invoke(
                app, ["config", "set", test_key, test_value], catch_exceptions=False
            )
            assert result.exit_code == 0, (
                f"Failed to set config: {result.stdout} {result.stderr}"
            )

            # Get the value in a separate invocation
            result = cli_runner.invoke(
                app, ["config", "get", test_key], catch_exceptions=False
            )
            assert result.exit_code == 0, (
                f"Failed to get config: {result.stdout} {result.stderr}"
            )
            ...
            config_path = (
                Path.home() / ".local-coding-assistant" / "config" / "config.json"
            )
    ...
    def test_cli_run_with_provider_setup(self, cli_runner, mock_bootstrap_success):
