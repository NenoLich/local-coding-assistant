...
class TestStreamingAPICompatibilityE2E:
    ...
    def test_event_buffering_into_traditional_results(self, cli_runner, mock_streaming_runtime_compat):
        ...
        event_sequence = [
            ...
            ExecutionEvent(type=EventType.TOOL_START, session_id=session_id, frame_id=frame_id,
                          data={"tool_call": MagicMock(name="calculator", arguments={"expression": "2+2"})}),
            ExecutionEvent(type=EventType.TOOL_RESULT, session_id=session_id, frame_id=frame_id,
                          data={"tool_call": MagicMock(name="calculator"),
            ...
            ExecutionEvent(type=EventType.TURN_COMPLETE, session_id=session_id,
                          data={"final_answer": "Buffered result: This is buffered content for compatibility. Calculator result: 4",
                                "report": {
                                    "message": "Buffered result: This is buffered content for compatibility. Calculator result: 4",
                                    "tool_calls": [{"name": "calculator", "arguments": {"expression": "2+2"}, "result": 4}]
                                }}),
        ]
    ...
    def test_non_streaming_consumers_work_with_event_streams(self, mock_streaming_runtime_compat):
        ...
        event_sequence = [
            ...
            ExecutionEvent(type=EventType.FRAME_COMPLETE, session_id=session_id, frame_id=frame_id),
            ExecutionEvent(type=EventType.TURN_COMPLETE, session_id=session_id,
                          data={"final_answer": "Non-streaming consumer test completed",
    ...
    def test_configuration_flag_handling_streaming_enabled(self, cli_runner, mock_streaming_runtime_compat):
        ...
        event_sequence = [
            ...
            ExecutionEvent(type=EventType.FRAME_COMPLETE, session_id=session_id, frame_id=frame_id),
            ExecutionEvent(type=EventType.TURN_COMPLETE, session_id=session_id,
                          data={"final_answer": "Streaming configuration test completed",
    ...
    def test_configuration_flag_handling_backward_compatibility(self, cli_runner, mock_streaming_runtime_compat):
        ...
        event_sequence = [
            ...
            ExecutionEvent(type=EventType.FRAME_COMPLETE, session_id=session_id, frame_id=frame_id),
            ExecutionEvent(type=EventType.TURN_COMPLETE, session_id=session_id,
                          data={"final_answer": "Backward compatibility test completed",
        ...
        with patch("local_coding_assistant.cli.commands.run.bootstrap") as mock_bootstrap:
            ...
            result = cli_runner.invoke(app, [
                "run", "query", "Test backward compatibility",
                "--model", "gpt-4",
                "--format", "plain"
            ])
    ...
    def test_event_stream_buffering_preserves_data_integrity(self, mock_streaming_runtime_compat):
        ...
        event_sequence = [
            ...
            ExecutionEvent(type=EventType.LLM_CHUNK, session_id=session_id, frame_id=frame_id,
                          data={"content": "Complex data processing ", "metadata": complex_data, "is_final": True}),
            ExecutionEvent(type=EventType.LLM_COMPLETE, session_id=session_id, frame_id=frame_id,
                          data={"total_tokens": 100, "usage_stats": complex_data}),
            ExecutionEvent(type=EventType.FRAME_COMPLETE, session_id=session_id, frame_id=frame_id,
                          data={"result": "integrity_verified", "final_data": complex_data}),
            ExecutionEvent(type=EventType.TURN_COMPLETE, session_id=session_id,
                          data={"final_answer": "Data integrity test completed",
                                "report": {"message": "Data integrity test completed", "complex_data": complex_data}}),
        ]
        ...
        llm_complete = next(e for e in collected_events if e.type == EventType.LLM_COMPLETE)
        assert llm_complete.data["usage_stats"] == complex_data

        turn_complete = next(e for e in collected_events if e.type == EventType.TURN_COMPLETE)
    ...
    def test_configuration_flags_affect_streaming_behavior(self, cli_runner, mock_streaming_runtime_compat):
        ...
        for agent_mode, tool_mode in test_configs:
            ...
            event_sequence = [
                ...
                ExecutionEvent(type=EventType.LLM_START, session_id=session_id, frame_id=frame_id),
                ExecutionEvent(type=EventType.LLM_CHUNK, session_id=session_id, frame_id=frame_id,
                              data={"content": f"Testing {agent_mode} mode with {tool_mode} tool calling", "is_final": True}),
                ExecutionEvent(type=EventType.LLM_COMPLETE, session_id=session_id, frame_id=frame_id),
                ExecutionEvent(type=EventType.FRAME_COMPLETE, session_id=session_id, frame_id=frame_id),
                ExecutionEvent(type=EventType.TURN_COMPLETE, session_id=session_id,
                              data={"final_answer": f"Configuration test completed: {agent_mode} {tool_mode}",
                                    "report": {"message": f"Configuration test completed: {agent_mode} {tool_mode}"}}),
            ]
            ...
            with patch("local_coding_assistant.cli.commands.run.bootstrap") as mock_bootstrap:
                ...
                result = cli_runner.invoke(app, [
                    "run", "query", f"Test {agent_mode} {tool_mode}",
                ...
                expected_content = f"Testing {agent_mode} mode with {tool_mode} tool calling"
                assert expected_content in result.stdout

                expected_final = f"Configuration test completed: {agent_mode} {tool_mode}"
