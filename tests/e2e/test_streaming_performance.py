...
class MockStreamingRuntimeWithMetrics:
    ...
    def __init__(
        self, event_sequence, delay_between_events=0.01, metrics_collector=None
    ):
...
def mock_streaming_runtime_with_metrics(performance_metrics):
    ...
    def _create_runtime(event_sequence, delay=0.01):
        return MockStreamingRuntimeWithMetrics(
            event_sequence, delay, performance_metrics
        )
...
class TestStreamingPerformanceE2E:
    ...
    def test_streaming_latency_improvement(
        self, cli_runner, mock_streaming_runtime_with_metrics, performance_metrics
    ):
        ...
        event_sequence.extend(
            [
                ...
                ExecutionEvent(
                    ...
                    session_id=session_id,
                    data={
                        "final_answer": "Streaming latency test completed",
                        "report": {"message": "Streaming latency test completed"},
                    },
                ),
            ]
        )

        runtime = mock_streaming_runtime_with_metrics(
            event_sequence, delay=0.02
        ...
        with patch(
            ...
            assert metrics["duration"] < 2.0, (
                f"Test took too long: {metrics['duration']}s"
            )
    ...
    def test_memory_usage_large_responses(
        self, cli_runner, mock_streaming_runtime_with_metrics, performance_metrics
    ):
        ...
        event_sequence.extend(
            [
                ...
                ExecutionEvent(
                    ...
                    session_id=session_id,
                    data={
                        "final_answer": "Large content test completed",
                        "report": {"message": "Large content test completed"},
                    },
                ),
            ]
        )
        ...
        with patch(
            ...
            result = cli_runner.invoke(
                app, ["run", "query", "Test memory usage with large content"]
            )
            ...
            assert content_length > 1000, (
                f"Content too short for large response test: {content_length} characters"
            )

            # Check memory usage - should be reasonable even with large content
            assert metrics["peak_memory_mb"] < 500, (
                f"Memory usage too high: {metrics['peak_memory_mb']} MB"
            )
            assert metrics["avg_memory_mb"] < 300, (
                f"Average memory usage too high: {metrics['avg_memory_mb']} MB"
            )

            # Should complete within reasonable time despite large content
            assert metrics["duration"] < 3.0, (
                f"Large content processing took too long: {metrics['duration']}s"
            )

    def test_responsiveness_under_load(
        self, cli_runner, mock_streaming_runtime_with_metrics, performance_metrics
    ):
        ...
        for i in range(3):  # Run 3 concurrent-like operations
            ...
            event_sequence = [
                ...
                ExecutionEvent(
                    ...
                    session_id=session_id,
                    data={
                        "final_answer": f"Load test {i} completed",
                        "report": {"message": f"Load test {i} completed"},
                    },
                ),
            ]
        ...
        assert avg_duration_per_operation < 1.0, (
            f"Average operation time too slow: {avg_duration_per_operation}s"
        )

        # Memory usage should remain stable under load
        assert metrics["peak_memory_mb"] < 400, (
            f"Memory usage too high under load: {metrics['peak_memory_mb']} MB"
        )

        # CPU usage should be reasonable
        assert metrics["avg_cpu_percent"] < 50, (
            f"CPU usage too high under load: {metrics['avg_cpu_percent']}%"
        )

    def test_streaming_vs_non_streaming_latency_comparison(
        self, cli_runner, mock_streaming_runtime_with_metrics, performance_metrics
    ):
        ...
        event_sequence.extend(
            [
                ...
                ExecutionEvent(
                    ...
                    session_id=session_id,
                    data={
                        "final_answer": "Latency comparison completed",
                        "report": {"message": "Latency comparison completed"},
                    },
                ),
            ]
        )
        ...
        with patch(
            ...
            result = cli_runner.invoke(
                app, ["run", "query", "Compare streaming vs non-streaming latency"]
            )
            ...
            estimated_non_streaming_duration = (
                len(content_chunks) * 0.1
            )  # Simulated non-streaming delay per chunk
