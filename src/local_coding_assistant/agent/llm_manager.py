...
class ToolCall(BaseModel):
    ...
    id: str | None = None
    name: str
    type: str = "function"  # "function" or "code"
...
class LLMManager:
    ...
    async def _handle_generation_response(
        ...
        try:
            ...
            # 1. Parse explicit tool calls from provider
            if provider_response.tool_calls:
                for tc in provider_response.tool_calls:
                    # Handle different provider formats if necessary
            ...
            # 2. Parse code blocks for Programmatic Tool Calling (PTC)
            # Only if no explicit tool calls were found (to avoid ambiguity)
            if not tool_calls and provider_response.content:
                code_tool_call = self._parse_code_blocks(provider_response.content)
                if code_tool_call:
                    tool_calls.append(code_tool_call)
    ...
    def _parse_code_blocks(self, content: str) -> ToolCall | None:
        """Parse markdown code blocks for programmatic tool calling.
        ...
        if match:
            code = match.group(1)
            return ToolCall(
                name="python_interpreter", type="code", arguments={"code": code}
            )
